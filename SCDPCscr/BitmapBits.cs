...
namespace SCDPCscr
{
    ...
    public class BitmapBits
    {
        ...
        public static BitmapBits FromBitmap(Bitmap bmp)
        {
        ...
        private static void LoadBitmap16BppArgb1555(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                ...
                for (int x = 0; x < bmp.Width; x++)
                {
                    ...
                    int B = pix & 0x1F;
                    B = B << 3 | B >> 2;
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(A, R, G, B).FindNearestMatch(MainForm.palslice));
                    if (A < 128)
                        bmp[x, y] = 0;
                }
            }
        ...
        private static void LoadBitmap16BppGrayScale(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                ...
                {
                    ushort pix = BitConverter.ToUInt16(Bits, srcaddr + (x * 2));
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(pix >> 8, pix >> 8, pix >> 8).FindNearestMatch(MainForm.palslice));
                }
            }
        }
        ...
        private static void LoadBitmap16BppRgb555(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                ...
                for (int x = 0; x < bmp.Width; x++)
                {
                    ...
                    int B = pix & 0x1F;
                    B = B << 3 | B >> 2;
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(R, G, B).FindNearestMatch(MainForm.palslice));
                }
            }
        }
        ...
        private static void LoadBitmap24BppRgb(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                int srcaddr = y * Math.Abs(Stride);
                for (int x = 0; x < bmp.Width; x++)
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(Bits[srcaddr + (x * 3) + 2], Bits[srcaddr + (x * 3) + 1], Bits[srcaddr + (x * 3)]).FindNearestMatch(MainForm.palslice));
            }
        }
        ...
        private static void LoadBitmap32BppArgb(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                ...
                for (int x = 0; x < bmp.Width; x++)
                {
                    ...
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, col.FindNearestMatch(MainForm.palslice));
                }
            }
        }
        ...
        private static void LoadBitmap32BppRgb(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                int srcaddr = y * Math.Abs(Stride);
                for (int x = 0; x < bmp.Width; x++)
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(Bits[srcaddr + (x * 4) + 2], Bits[srcaddr + (x * 4) + 1], Bits[srcaddr + (x * 4)]).FindNearestMatch(MainForm.palslice));
            }
        }
        ...
        private static void LoadBitmap48BppRgb(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                int srcaddr = y * Math.Abs(Stride);
                for (int x = 0; x < bmp.Width; x++)
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(BitConverter.ToUInt16(Bits, srcaddr + (x * 6) + 4) / 255, BitConverter.ToUInt16(Bits, srcaddr + (x * 6) + 2) / 255, BitConverter.ToUInt16(Bits, srcaddr + (x * 6)) / 255).FindNearestMatch(MainForm.palslice));
            }
        }
        ...
        private static void LoadBitmap64BppArgb(BitmapBits bmp, byte[] Bits, int Stride)
        {
            for (int y = 0; y < bmp.Height; y++)
            {
                int srcaddr = y * Math.Abs(Stride);
                for (int x = 0; x < bmp.Width; x++)
                    bmp[x, y] = (byte)Array.IndexOf(MainForm.palslice, Color.FromArgb(BitConverter.ToUInt16(Bits, srcaddr + (x * 8) + 6) / 255, BitConverter.ToUInt16(Bits, srcaddr + (x * 8) + 4) / 255, BitConverter.ToUInt16(Bits, srcaddr + (x * 8) + 2) / 255, BitConverter.ToUInt16(Bits, srcaddr + (x * 8)) / 255).FindNearestMatch(MainForm.palslice));
            }
        }
